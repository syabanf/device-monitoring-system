          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                {DEFAULT_SENSORS.map((s) => { const available = isSensorTypeEnabled(s.type); return (
                  <label key={s.type} className={cn('flex items-center justify-between rounded-2xl bg-surface p-3', !available && 'opacity-50')}><span><span className="block text-sm font-medium">{s.name}</span><span className="block text-xs text-muted">{SENSOR_TYPE_LABEL[s.type]} · {available ? `${s.portKind} port` : 'not available yet'}</span></span><Toggle checked={available && !!enabled[s.type]} disabled={!available} onCheckedChange={(v) => setEnabled({ ...enabled, [s.type]: v })} label={s.name} /></label>
                ); })}
              </div>
