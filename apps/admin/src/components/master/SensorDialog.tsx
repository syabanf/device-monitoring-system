...
export function emptySensor(device: Device): Sensor {
  // Temperature sensors sit on a digital port, so a new one starts on the first free digital one.
  const free = device.ports.find((p) => !p.sensorId && p.kind === 'digital') ?? device.ports.find((p) => !p.sensorId) ?? device.ports[0]!;
  return { id: '', deviceId: device.id, outletId: device.outletId, name: '', type: 'TEMPERATURE_HUMIDITY', portKind: free.kind, portIndex: free.index, unit: '°C', thresholds: { min: 18, max: 28, humidityMin: 30, humidityMax: 60 }, enabled: true, floor: { x: 50, y: 50 } };
}
      ...
      <DialogContent>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <FormField label="Type" hint="Door, motion, power and panic sensors are not part of this rollout yet."><Select value={s.type} onValueChange={(v) => set({ type: v as SensorType })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{(Object.keys(SENSOR_TYPE_LABEL) as SensorType[]).map((t) => <SelectItem key={t} value={t} disabled={!isSensorTypeEnabled(t) && t !== s.type}>{SENSOR_TYPE_LABEL[t]}</SelectItem>)}</SelectContent></Select></FormField>
