...
export function AnalysisPage() {
  ...
  const [sensorId, setSensorId] = React.useState(tempSensors[0]?.id ?? '');
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Analysis" description="Alert volume, employee response performance and environment trends" actions={
        <Select value={period} onValueChange={(v) => setPeriod(v as Period)}><SelectTrigger className="w-44"><SelectValue /></SelectTrigger><SelectContent>{PERIODS.map((p) => <SelectItem key={p.value} value={p.value}>{p.label}</SelectItem>)}</SelectContent></Select>
      } />
