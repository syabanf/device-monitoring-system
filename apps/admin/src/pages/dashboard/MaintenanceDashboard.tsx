...
export function MaintenanceDashboard() {
  ...
  const overdueChecks = devices.filter((d) => Date.parse(d.nextMaintenanceAt) < FIXTURE_NOW_MS).length;
  const warrantySoon = devices.filter((d) => Date.parse(d.warrantyUntil) < FIXTURE_NOW_MS + 90 * DAY).length;
  ...
  const upcoming = open.filter((t) => t.scheduledAt && Date.parse(t.scheduledAt) >= FIXTURE_NOW_MS).sort((a, b) => a.scheduledAt!.localeCompare(b.scheduledAt!));
