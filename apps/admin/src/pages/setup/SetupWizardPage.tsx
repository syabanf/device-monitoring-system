...
const STEPS = [
  ...
  { key: 'devices', title: 'AKCP units', icon: Cpu },
...
export function SetupWizardPage() {
  ...
  const [outlets, setOutlets] = React.useState<OutletDraft[]>([{ key: newId('o'), code: 'IDM-SBY-0021', name: 'Indomaret ', address: '', city: 'Surabaya', model: deviceTypes[0]?.model ?? 'SP1+', sensors: ['TEMPERATURE_HUMIDITY'] }]);
  const [people, setPeople] = React.useState<EmployeeDraft[]>([]);
  const [integ, setInteg] = React.useState({ apiBaseUrl: '', telegram: false });
  React.useEffect(() => {
    if (config) setInteg({ apiBaseUrl: config.apiBaseUrl, telegram: config.telegram.enabled });
  }, [config]);
  ...
  const strip = (c: NonNullable<typeof config>) => ({ apiBaseUrl: c.apiBaseUrl, telegram: c.telegram, push: c.push });
  ...
  const finish = async () => {
    ...
    if (config) await saveConfig({ ...strip(config), apiBaseUrl: integ.apiBaseUrl, telegram: { ...config.telegram, enabled: integ.telegram } });
    ...
    <div className="mx-auto max-w-5xl space-y-4">
      ...
      <Card>
        <CardContent className="p-6">
          ...
          {step === 1 ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between"><h2 className="text-lg font-bold">Add the outlets to monitor</h2><Button size="sm" variant="outline" onClick={() => setOutlets([...outlets, { key: newId('o'), code: `IDM-SBY-${String(21 + outlets.length).padStart(4, '0')}`, name: 'Indomaret ', address: '', city: 'Surabaya', model: deviceTypes[0]?.model ?? 'SP1+', sensors: ['TEMPERATURE_HUMIDITY'] }])}><Plus />Add row</Button></div>
          ...
          {step === 2 ? (
            <div className="space-y-4">
              <h2 className="text-lg font-bold">Choose the AKCP unit and sensors per outlet</h2>
              <div className="space-y-2">
          ...
          {step === 4 ? (
            <div className="space-y-4">
              <h2 className="text-lg font-bold">Connect the units</h2>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <FormField label="MQTT topic the API subscribes to" htmlFor="w-topic"><Input id="w-topic" readOnly value={config?.mqtt.topicFilter ?? 'spp/+/sensor/+/+'} className="[&_input]:bg-surface [&_input]:font-mono [&_input]:text-xs" /></FormField>
                <FormField label="Broker the API listens on" htmlFor="w-broker" className="sm:col-span-2" hint="Set each unit's MQTT broker to this machine's address on the outlet network, port 1883. The broker itself lives in the API environment as MQTT_BROKER_URL."><Input id="w-broker" readOnly value={config?.mqtt.brokerUrl || 'MQTT_BROKER_URL is not set'} className="[&_input]:bg-surface [&_input]:font-mono [&_input]:text-xs" /></FormField>
              </div>
          ...
          {step === 5 ? (
            <div className="flex flex-col items-center py-6 text-center">
              ...
              <p className="mt-1 max-w-md text-sm text-muted">{created ? `${created.outlets} outlet(s), ${created.devices} AKCP unit(s) and ${created.employees} employee(s) were created for ${org.name}.` : 'Nothing was created.'} Readings and alerts appear on the dashboard as soon as the units publish to the broker.</p>
