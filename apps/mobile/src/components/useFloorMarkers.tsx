...
export function useFloorMarkers(outletIds: string[], outletId: string | null | undefined): FloorMarker[] {
  ...
  return React.useMemo(() => {
    ...
    for (const d of devicesByOutlet.get(outletId) ?? []) {
      ...
      out.push({ id: d.id, x: d.floor.x, y: d.floor.y, kind: 'device', label: `Room Alert ${d.model.replace('RA', '')}`, sublabel: d.serial, status: d.status === 'offline' ? 'offline' : h.status === 'critical' ? 'fault' : 'normal', icon: <Router /> });
