...
export function PhotoDropzone({ urls, onChange, onBusyChange }: { urls: string[]; onChange: (urls: string[]) => void; onBusyChange?: (busy: boolean) => void }) {
  ...
  const add = async (files: FileList | null) => {
    ...
    try {
      ...
      setError('A photo could not be prepared. Please try another image.');
      setProgress(null);
    } finally {
      onBusyChange?.(false);
    }
  ...
  return (
    <div>
      ...
      {urls.length ? <div className="mb-3 grid grid-cols-3 gap-2">{urls.map((u) => <div key={u} className="relative aspect-square overflow-hidden rounded-2xl bg-surface">
        <img src={u} alt="Response evidence" className="size-full object-cover" />
        <span className="absolute bottom-1.5 left-1.5 flex size-6 items-center justify-center rounded-full bg-emerald-500 text-white" aria-label="Photo ready"><Check className="size-3.5" /></span>
      ...
      <div className={cn('flex flex-col items-center justify-center gap-3 rounded-[22px] border-2 border-dashed border-silver/70 bg-surface p-6 text-center', (urls.length >= 4 || busy) && 'opacity-60')}>
        ...
        <div><p className="text-xs font-semibold text-body" aria-live="polite">{busy ? `Preparing photos · ${progress}%` : urls.length ? `${urls.length} of 4 photos ready` : 'Add a photo of the triggered sensor'}</p><p className="mt-1 text-[11px] text-muted">Photos are attached when you submit your response.</p></div>
