...
export function deviceHealth(d: Device): { status: HealthStatus; issues: HealthIssue[] } {
  ...
  if (next < nowMs()) issues.push({ code: 'maintenance_overdue', label: `Preventive check overdue ${Math.ceil((nowMs() - next) / DAY)}d`, severity: 'attention' });
  else if (next < nowMs() + 30 * DAY) issues.push({ code: 'maintenance_due', label: `Preventive check due in ${Math.ceil((next - nowMs()) / DAY)}d`, severity: 'attention' });
  ...
  if (warranty < nowMs()) issues.push({ code: 'warranty', label: 'Warranty expired', severity: 'attention' });
  else if (warranty < nowMs() + 90 * DAY) issues.push({ code: 'warranty', label: `Warranty ends in ${Math.ceil((warranty - nowMs()) / DAY)}d`, severity: 'attention' });
...
export const isTicketOverdue = (t: MaintenanceTicket) => isTicketOpen(t) && !!t.scheduledAt && Date.parse(t.scheduledAt) < nowMs();
